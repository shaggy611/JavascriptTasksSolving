...
const arraySortInfo = (array) => {
    let growth = array.every((element, index) => index === 0 || element >= array[index - 1]);
    let decline = array.every((element, index) => index === 0 || element <= array[index - 1]);

    if(array.some((element) => typeof element !== 'number')) {
        return 'One of the elements is not a Number'
    } else {
        if (growth) {
            return 'Sort by Growth'
        }
    
        if (decline) {
            return 'Sort by Decline'
        }
    }
