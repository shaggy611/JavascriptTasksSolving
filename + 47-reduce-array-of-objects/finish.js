...
function popularPostsIds(posts, minimalComentsQty) {
  return posts.reduce((arrIds, current) => {
    if(current.comments >= minimalComentsQty) {
      return arrIds.concat([current.postId])
    } 
...
// function popularPostsIds(posts, minimalComentsQty) {
//   return posts.reduce((arrIds, current) => current.comments >= minimalComentsQty ? arrIds.push(current.postId) : arrIds, [])
// }
